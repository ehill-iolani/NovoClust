# save as find_identical_fast.py
...
# Get data paths
data = os.listdir("./cluster_analysis/variants_over1pct")
...
if not os.path.exists("./compare_analysis"):
    os.makedirs("./compare_analysis")        

if not os.path.exists("./compare_analysis/cleaned_data"):
    os.makedirs("./compare_analysis/cleaned_data")

if not os.path.exists("./compare_analysis/aligned_data"):
    os.makedirs("./compare_analysis/aligned_data")

if not os.path.exists("./compare_analysis/clustered_data"):
    os.makedirs("./compare_analysis/clustered_data")

if not os.path.exists("./compare_analysis/combined_cleaned_data"):
    os.makedirs("./compare_analysis/combined_cleaned_data")

# Loop through the list of file names and rename headers for each file
for file_name in file_names:
    input_fasta = os.path.join("./cluster_analysis/variants_over1pct", file_name)
    output_fasta = os.path.join("./compare_analysis/cleaned_data", file_name)
    rename_fasta_headers(input_fasta, output_fasta)

# Combine all the cleaned fasta files into one file
with open("./compare_analysis/combined_cleaned_data/combined.fasta", "w") as combined_handle:
    for file_name in file_names:
        cleaned_fasta = os.path.join("./compare_analysis/cleaned_data", file_name)
...
os.system("mafft --preservecase --auto ./compare_analysis/combined_cleaned_data/combined.fasta > ./compare_analysis/aligned_data/aligned.fasta")

# end trim the alignment
os.system("python scripts/endtrim_alignment.py ./compare_analysis/aligned_data/aligned.fasta ./compare_analysis/aligned_data/aligned_trimmed.fasta")

# ungap the trimmed alignment
os.system("python scripts/ungap_fasta.py ./compare_analysis/aligned_data/aligned_trimmed.fasta ./compare_analysis/aligned_data/aligned_trimmed_ungapped.fasta")

# Perform clustering on the trimmed alignment
os.system("vsearch --uc ./compare_analysis/clustered_data/derep.uc --derep_fulllength ./compare_analysis/aligned_data/aligned_trimmed_ungapped.fasta --output ./compare_analysis/clustered_data/derep.fasta --sizeout")

# Read in the UC file
clusters = defaultdict(list)
with open("./compare_analysis/clustered_data/derep.uc", "r") as uc_file:
    for line in uc_file:
        if line.startswith("#"):
            continue
...
with open("./compare_analysis/clustered_data/clusters_with_multiple_sequences.txt", "w") as output_file:
    for cluster_id, seq_ids in clusters_with_multiple_sequences.items():
